  ...
  return {
    ...
    async postBuild({
      ...
      const addToSearchData = (d) => {
        ...
        let version = docVersions.length ? docVersions[0] : "6.x.x";

        const matchedVersion = d.url.match(/docs\/(\d+\.x\.x)/);
        //
        if (!!matchedVersion && matchedVersion.length) {
          version = matchedVersion[1];
        } else
          version = d.url.includes("/next/") ? "next" : docVersions[0] || "6.x.x";
...
function buildSearchData(files, addToSearchData) {
  ...
  console.log(
    `docusaurus-lunr-search:: Start scanning documents in ${Math.min(
      workerCount,
      files.length
    )} threads`
  );
