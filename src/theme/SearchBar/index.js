...
const Search = (props) => {
  ...
  useEffect(() => {
    ...
    setVersionToSearch(
      determineDocsVersionFromURL(location.pathname, baseUrl, "docs", versions)
    );
  }, [location, baseUrl, versions]);

  const initAlgolia = (searchDocs, searchIndex, DocSearch, versionToSearch) => {
    new DocSearch({
      searchDocs,
      searchIndex,
      ...
      versionsToSearch: [versionToSearch],
    });
  };
  ...
  const loadAlgolia = () => {
    if (!initialized.current) {
      Promise.all([
        ...
        initAlgolia(searchDocs, searchIndex, DocSearch, versionToSearch);
      });
  ...
  return ( <
    ...
    key = "search-box" >
    <
    span aria - label = "expand searchbar"
    role = "button"
    className = {
      classnames("search-icon", {
    ...
    /> <
    input id = {
      props.searchBarId || "search_input_react"
    }
    type = "search"
    placeholder = {
    ...
    onFocus = {
      true ? null : toggleSearchIconClick
    }
    onBlur = {
      true ? null : toggleSearchIconClick
    }
